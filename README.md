...
The primary goal of Eden is to eliminate the common hassle when it comes to over-complicated Roblox frameworks. Eden keeps it lean and simple by providing a flexible, essentialistic foundation for you and your team to build your project in a rapid iteration environment such as Roblox. 
...
Eden is designed to be as predicatable as possible, meaning theres no room for unexpected behavior or obscure edge cases.
...
Another useful feature that exists in Eden is the auto initialization of functions in priority order. 
...
Silent cyclical hangs are unfortunate, as they cause your game to break silently and can take time to find. Thankfully, Eden will automatically detect and warn you if this occurs.

NOTE: Due to Luau limitations, a module which surpasses the `LOAD_TIMEOUT` time will lose any Luau optimizations during that run session.

*Check out the Order framework if you're into cyclicals (https://github.com/michaeldougal/order)*

- **Hang detection**

Sometimes code can infinitely yield silently, and it's important to know when this issue occurs. Eden monitors the execution times of your modules and will warn you if one is taking a bit too long.
...
An `int` that specifies the order in which the `:Init()` method is called on game start. The higher the priority the sooner it will run. Negative `int`s are allowed as well and will run after everything else.
...
Eden is designed to have as few quirks as possible while still giving you a reliable foundation to work on.
...
For example, I want module X to not be required on game start because I'm going to require it separately later. I can put it under a folder called `Static` under any of the root project directories such as `Client` and it will not be required. This is also great for things like `roact` which has a crazy amount of unused modules.
...
1. Download the source code by pressing the `Code` button at the top of the repositoy and pressing "Download ZIP"
2. Unzip the file and open the "src" folder
3. In studio, create a folder under `ServerScriptService` called "ServerModules", this folder is where you will put all of your server code **modules**.
...
5. In studio, create a folder under `ReplicatedStorage` called "SharedModules", this folder is where you will put all of your code that needs to be shared between the client and server.
